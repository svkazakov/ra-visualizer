...
class Visualizer(object):
    def __init__(self, ref_SIZE=None, ref_file=None, contigs_FN=None, reads_FN=None, view_pos_from=0, view_size=6000000,
                 output_FN="output.svg", output_read_projection=False, levels_for_gap_labels=2):
        ...
        self.output_read_projection = output_read_projection
        self.levels_for_gap_labels = levels_for_gap_labels
    ...
    def calculate_reference_size(self):
        if not self.ref_SIZE:
            self.ref_SIZE = calculate_one_contig_size(self.ref_file)
    ...
    def draw_contigs(self):
        ...
        inside = 0
        last_end = None
        last_gap_coord_at_level = [None] * (self.levels_for_gap_labels - 1)
        for contig in contigs:
            ...
            if (ref_end < self.view_POS_FROM) or (ref_start > self.view_POS_TO):
                ...
                if last_end is not None:
                    ...
                    level = 0       # label level
                    while level < self.levels_for_gap_labels - 1:
                        if (last_gap_coord_at_level[level] is None) or (gap_middle - last_gap_coord_at_level[level] >= 70000):
                            # using this level
                            break
                        level += 1
                    for x in range(level):        # moving to lower position
                        text_y -= 0.3
                        path_y = text_y+0.25
                    ...
                    if level < self.levels_for_gap_labels - 1:
                        last_gap_coord_at_level[level] = gap_middle
                last_end = ref_end
    ...
    def draw_reads(self):
        ...
        for gr in [0,1,2]:
            for read in reads[gr]:
                ...
                while True:
                    if read.ref_start - (left if self.output_read_projection else 0) >= avail_from[y]:
                        ...
                        # else:
                        if self.output_read_projection:
                        ...
                        avail_from[y] = read.ref_end + reads_GAP_between[gr] + 1 + \
                                            (right if self.output_read_projection else 0)
                        # avail_from[y] = read.ref_end + reads_GAP_between[gr] + 1
                        printed = True
                        break
                    y += 1
...
class Read():
    def __init__(self, r, line, ref_size):
        ...
        interesting = (r[7] == "YES")
        useful      = (r[8] == "YES")
        clipped_head= int(r[9])
        clipped_tail= int(r[10])
...
def main():
    ...
    parser.add_argument("--ref-size", help="Reference size (Please set --ref-size or --ref-file)", type=int)
    parser.add_argument("--ref-file", help="Reference .fasta file (Please set --ref-size or --ref-file)", type=str)
    ...
    parser.add_argument("-f", "--from-pos", help="View window FROM coordinate (default: 0)", type=int, default=0)
    parser.add_argument("-s", "--view-size", help="View window size (default: 6'000'000)", type=int, default=6000000)

    parser.add_argument("--output-read-projection", action='store_true', help="Output read projection (default: False)")
    parser.add_argument("--levels-for-gap-labels", help="How many levels to use for outputting gap labels (default: 2)", type=int, default=2)

    args = parser.parse_args()
    Visualizer(args.ref_size, args.ref_file, args.contigs, args.reads, args.from_pos, args.view_size, args.output,
               args.output_read_projection, args.levels_for_gap_labels)\
        .run()
